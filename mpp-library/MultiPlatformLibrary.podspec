...
if [ ! -e "$MPP_OUTPUT_NAME" ]; then
  echo "framework not found at $MPP_OUTPUT_NAME, start compilation"
  "$MPP_PROJECT_ROOT/../gradlew" --no-daemon --configure-on-demand -p "$MPP_PROJECT_ROOT" "$GRADLE_TASK"
else
  echo "framework already compiled in gradle, use exists $MPP_OUTPUT_NAME"
fi
            SCRIPT
        }
    ]
